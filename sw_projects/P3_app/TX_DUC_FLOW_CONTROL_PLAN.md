...
`P3_app` TX DUC traffic is sensitive to short bursts of scheduler jitter,
...
currently expose queue-credit pacing from `P3_app` back to the client. Within
...
- if protocol changes are acceptable later, add explicit queue-credit feedback
  so the client can pace TX toward `P3_app` instead of relying only on app-side
