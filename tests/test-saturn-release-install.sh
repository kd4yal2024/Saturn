...
# shellcheck disable=SC2016
grep -Fq 'find "$SATURN_STATE_DIR" \( -path "$SATURN_RELEASE_STAGING_DIR" -o -path "$SATURN_REPOSITORY_RESTORE_DIR" -o -path "$SATURN_DEPLOYMENT_STATE_DIR" \) -prune -o -type d' \
  "$REPO_ROOT/update_manager/install_saturn_go_nginx.sh"
# shellcheck disable=SC2016
grep -Fq 'find "$SATURN_STATE_DIR" \( -path "$SATURN_RELEASE_STAGING_DIR" -o -path "$SATURN_REPOSITORY_RESTORE_DIR" -o -path "$SATURN_DEPLOYMENT_STATE_DIR" \) -prune -o -type f' \
  "$REPO_ROOT/update_manager/install_saturn_go_nginx.sh"
