...
| `saturn-xdma-operational-rx-smoke.sh` | Appliance acceptance harness for direct XDMA. It requires a 192 kHz RX rate, exercises TCI IQ/audio, DSP, retuning, and RF-inhibited H2C/DUC TX through direct or split-proxy transport, verifies receive-safe cleanup, and restores prior services. | `--duration-seconds SECONDS`, `--client-probe`, `--proxy-client-probe` |
| `saturn-xdma-operational-client.py` | Localhost TCI acceptance client validating IQ/audio, DSP/retune continuity, RF-inhibited DUC writes with no keying or FIFO fault, and disconnect cleanup. | `--url`, `--media-url`, `--readiness-file`, `--retune-hz`, `--timeout-seconds`, `--basic-auth-systemd-unit`, `--insecure-tls`, `--self-test` |
