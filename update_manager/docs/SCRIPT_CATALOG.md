...
| `saturn-radio-backend-switch-root.sh` | Root-owned transactional appliance-wide radio ownership broker. It stops the bridge at the receive-safe release boundary, prevents concurrent P2/XDMA ownership, applies a backend-specific systemd drop-in, persists only a ready selection, and restores the prior services, drop-in, and state after failure. Direct XDMA activation remains disabled in the installed root-owned policy while that backend is probe-only. | `status`, `switch p2`, `switch xdma` (XDMA disabled by policy) |
