...
| `saturn-xdma-operational-rx-smoke.sh` | Source-tree appliance acceptance harness for the RX-only direct-XDMA runtime. It rejects stale binaries, requires advancing DMA/IQ readiness, verifies signal-driven receive-safe cleanup, and restores the prior P2/bridge service activity. It never changes the production backend-selection policy. | `--duration-seconds SECONDS` |
