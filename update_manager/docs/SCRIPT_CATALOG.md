...
| `saturn-release-activate-root.sh` | Root-owned REM-0203 broker that validates an installed full commit, persists an activation transaction, atomically switches `/opt/saturn/current`, restarts affected services in dependency order, and requires exact-commit readiness. Production activation is disabled until REM-0204 rollback is ready. | `--validate FULL_COMMIT`, `FULL_COMMIT` (disabled by default) |
