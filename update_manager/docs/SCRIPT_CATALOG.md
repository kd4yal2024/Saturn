...
| `saturn-radio-backend-switch-root.sh` | Root-owned transactional appliance-wide radio ownership broker. It stops the bridge at the receive-safe release boundary, prevents concurrent P2/XDMA ownership, applies a backend-specific systemd drop-in, requires fresh and advancing direct-RX counters for XDMA, persists only a ready selection, and restores the prior services, drop-in, and state after failure. Direct XDMA production selection remains disabled while the RX-only runtime completes appliance validation. | `status`, `switch p2`, `switch xdma` (XDMA disabled by policy) |
| `saturn-xdma-operational-rx-smoke.sh` | Source-tree appliance acceptance harness for the RX-only direct-XDMA runtime. It rejects stale binaries, requires a 192 kHz steady-state rate, can exercise TCI IQ/audio, DSP settings, retuning, TX refusal, and disconnect either directly or through Saturn Go's authenticated TLS split proxy, verifies signal-driven receive-safe cleanup, and restores the prior P2/bridge service activity. It never changes the production backend-selection policy. | `--duration-seconds SECONDS`, `--client-probe`, `--proxy-client-probe` |
| `saturn-xdma-operational-client.py` | Localhost-only RFC 6455/TCI acceptance client used by the operational RX smoke harness. It supports direct WebSocket and paired TLS control/media transport, validates 192 kHz IQ and 48 kHz audio frames, continued streaming after a DSP preference burst, a 7.200 MHz retune, advancing safe readiness, and fail-closed TX refusal without external Python packages. | `--url`, `--media-url`, `--readiness-file`, `--retune-hz`, `--timeout-seconds`, `--basic-auth-systemd-unit`, `--insecure-tls`, `--self-test` |
| `saturn-xdma-backend-switch-smoke.sh` | Bounded appliance-only transaction acceptance. Starting from stable P2 ownership, it stages a fresh source-tree XDMA binary in a temporary root-owned directory under `/opt/saturn-go` for compatibility with the hardened service namespace and the appliance's `noexec` `/run`, temporarily selects it through the broker's explicit test gate, runs the Saturn Go split-proxy client acceptance, returns ownership to P2, and restores the exact prior drop-ins/state. Failures include bounded unit/readiness/journal diagnostics. Production XDMA selection remains disabled. | none |
