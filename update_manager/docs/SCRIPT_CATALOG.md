...
| `saturn-release-activate-root.sh` | Root-owned REM-0203/0204 broker that validates an installed full commit, snapshots the prior pointer/service configuration, persists an activation transaction, atomically switches `/opt/saturn/current`, restarts affected services in dependency order, and requires exact-commit readiness. Any post-prepare failure automatically restores and verifies the prior release; incomplete recovery is persisted as `rollback_failed`. Production activation remains disabled pending an approved appliance test. | `--validate FULL_COMMIT`, `FULL_COMMIT` (disabled by default) |
