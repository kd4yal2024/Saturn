...
| `update-pihpsdr.py` | `1.13` | Update/build piHPSDR with dependency preflight, current PipeWire headers, and WDSP 2.00 Linux compatibility. | `--skip-git`, `-y`, `-n`, `--no-gpio`, `--dry-run`, `--verbose` |
...
| `saturn-rust-toolchain.sh` | Install or verify the repository-pinned Rust build prerequisite using a checksummed immutable rustup bootstrap. | `ensure`, `check`, `print-config` |
