...
| `saturn-xdma-operational-rx-smoke.sh` | Source-tree appliance acceptance harness for the RX-only direct-XDMA runtime. It rejects stale binaries, requires a 192 kHz steady-state rate, can exercise TCI IQ/audio, DSP settings, retuning, TX refusal, and disconnect through the dependency-free client probe, verifies signal-driven receive-safe cleanup, and restores the prior P2/bridge service activity. It never changes the production backend-selection policy. | `--duration-seconds SECONDS`, `--client-probe` |
| `saturn-xdma-operational-client.py` | Localhost-only RFC 6455/TCI acceptance client used by the operational RX smoke harness. It validates 192 kHz IQ and 48 kHz audio frames, continued streaming after a DSP preference burst, a 7.200 MHz retune, advancing safe readiness, and fail-closed TX refusal without external Python packages. | `--url`, `--readiness-file`, `--retune-hz`, `--timeout-seconds`, `--self-test` |
