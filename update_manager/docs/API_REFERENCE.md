...
- `app_telemetry.current` includes:
  ...
  - FIFO/ADC gauges
  - `gauges.fpga_fifo_v29`, when supported: build marker, coherent snapshot
    validity/generation/timeout count, and raw DDC/DUC/microphone/speaker
    occupancy/minimum/maximum word counts plus aggregate event-transition
    counters. These FPGA counters combine overflow-signal, full, and empty
    transitions and are not sample-loss counters.
