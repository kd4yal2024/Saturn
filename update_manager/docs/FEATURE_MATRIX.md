...
| Hidden P2/P3 app build/deploy/switch/revert test lab | `p23test.html` (hidden/no nav link) | `POST /run`, `GET /run_log` (with `script=p23-app-manager.sh`) | `p23-app-manager.sh`, `make`, `systemctl`, systemd drop-in override | In-memory run-log buffer; `/opt/saturn-go/p23-apps`; `/etc/systemd/system/p2app.service.d/10-saturn-p23-switch.conf` |
| Hidden P2/P3 status panel | `p23test.html` (hidden/no nav link) | `GET /p23_status` | Backend filesystem + `systemctl` inspection | N/A |
