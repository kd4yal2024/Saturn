...
| Dedicated deskHPSDR terminal runner | `deskhpsdr.html` | `POST /run`, `GET /run_log` (with `script=update-deskhpsdr.py`) | `update-deskhpsdr.py`, `scripts/deskhpsdr-test-build-on-current-image.sh`, `scripts/patches/deskhpsdr-libgpiod-v2.patch` from active repo root | In-memory run-log buffer |
