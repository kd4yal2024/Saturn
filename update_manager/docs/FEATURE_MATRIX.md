...
| Dedicated deskHPSDR terminal runner | `deskhpsdr.html` | `POST /run`, `GET /run_log` (with `script=update-deskhpsdr.py`) | `update-deskhpsdr.py`, `scripts/deskhpsdr-test-build-on-current-image.sh`, startup guard `scripts/patches/deskhpsdr-active-receiver-init.patch`, current upstream channel, and pinned `2.6.84` legacy V1 GPIO channel with `deskhpsdr-libgpiod-v2.patch` | In-memory run-log buffer |
...
| Fixed-window performance benchmark, history, and A/B no-regression verdict | Radio Telemetry (`p23test.html`) | `GET/POST /performance_benchmarks`, `POST /performance_benchmarks/compare`, `POST /performance_benchmarks/delete` | Validated live `p2app` host/app/XDMA samples with workload-identity lock, statistical summaries, event gates, provenance, notes, and operator observation | `/var/lib/saturn-state/performance_benchmarks.json` (newest 64 runs) |
