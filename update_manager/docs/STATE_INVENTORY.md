...
|---|---|---|
| `GET /backup_settings` | A versioned, checksummed selection of Saturn Remote settings/profiles, Performance Lab benchmark history, state schema, custom-script registry and registered operator scripts, update policy, and piHPSDR/deskHPSDR `*.props`. | Credentials, cookie/TLS/SSH secrets, Tailscale/network/host identity, boot and provisioning config, deployments/logs, source/releases/caches, and FPGA hardware state. |
