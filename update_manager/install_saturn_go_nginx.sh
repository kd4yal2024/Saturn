...
SATURN_APPLIANCE_POWER_NAME="saturn-appliance-power-root.sh"
...
SATURN_RESTORE_TRANSACTION_TOOL_NAME="saturn-restore-transaction.py"
SATURN_STATE_WRITER_TOOL_NAME="saturn-state-write.py"
SATURN_MAINTENANCE_LOCK_TOOL_NAME="saturn-maintenance-lock.py"
SATURN_MAINTENANCE_LOCK_DIR="${SATURN_MAINTENANCE_LOCK_DIR:-/run/lock/saturn-maintenance}"
SATURN_MAINTENANCE_TMPFILES="/etc/tmpfiles.d/saturn-maintenance.conf"
...
SATURN_REPOSITORY_RESTORE_DIR="${SATURN_REPOSITORY_RESTORE_DIR:-${SATURN_STATE_DIR}/repository-restores}"
...
# The bridge must build when required, but it is an operator-controlled runtime
# service in P2 mode and therefore is not a Saturn Go process-readiness gate.
SATURN_READY_REQUIRE_BRIDGE="${SATURN_READY_REQUIRE_BRIDGE:-0}"
...
PRIVILEGED_HELPER_SCRIPTS=(
  ...
  "$SOURCE_DIR/scripts/$SATURN_APPLIANCE_POWER_NAME"
  ...
  "$SOURCE_DIR/scripts/saturn-satp-config.sh"
...
mkdir -p "$BIN_DIR" "$SCRIPTS_DIR" "$WATCHDOG_SCRIPT_DIR" "$PRIVILEGED_SCRIPTS_DIR" "$WEB_ROOT" "$SATURN_STATE_DIR" "$SATURN_SNAPSHOT_DIR" "$SATURN_STAGING_DIR" "$SATURN_REPOSITORY_RESTORE_DIR" "$SATURN_RELEASE_STAGING_DIR"
...
if [[ "$SATURN_MAINTENANCE_LOCK_DIR" != /* || "$SATURN_MAINTENANCE_LOCK_DIR" =~ [[:space:]] ]]; then
  err "Maintenance lock directory must be an absolute path without whitespace: $SATURN_MAINTENANCE_LOCK_DIR"
  exit 1
fi
if [[ -L "$SATURN_MAINTENANCE_TMPFILES" ]]; then
  err "Refusing symlinked maintenance tmpfiles policy: $SATURN_MAINTENANCE_TMPFILES"
  exit 1
fi
cat >"$SATURN_MAINTENANCE_TMPFILES" <<EOF
d $SATURN_MAINTENANCE_LOCK_DIR 0750 root $SERVICE_GROUP -
f $SATURN_MAINTENANCE_LOCK_DIR/release.lock 0660 root $SERVICE_GROUP -
f $SATURN_MAINTENANCE_LOCK_DIR/repository.lock 0660 root $SERVICE_GROUP -
f $SATURN_MAINTENANCE_LOCK_DIR/disk.lock 0660 root $SERVICE_GROUP -
f $SATURN_MAINTENANCE_LOCK_DIR/fpga.lock 0660 root $SERVICE_GROUP -
f $SATURN_MAINTENANCE_LOCK_DIR/package.lock 0660 root $SERVICE_GROUP -
f $SATURN_MAINTENANCE_LOCK_DIR/network.lock 0660 root $SERVICE_GROUP -
f $SATURN_MAINTENANCE_LOCK_DIR/radio.lock 0660 root $SERVICE_GROUP -
f $SATURN_MAINTENANCE_LOCK_DIR/read-only.lock 0660 root $SERVICE_GROUP -
EOF
chown root:root "$SATURN_MAINTENANCE_TMPFILES"
chmod 0644 "$SATURN_MAINTENANCE_TMPFILES"
systemd-tmpfiles --create "$SATURN_MAINTENANCE_TMPFILES"
...
find "$SATURN_STATE_DIR" \( -path "$SATURN_RELEASE_STAGING_DIR" -o -path "$SATURN_REPOSITORY_RESTORE_DIR" -o -path "$SATURN_DEPLOYMENT_STATE_DIR" \) -prune -o -type d -print0 \
  | xargs -0 -r chmod 0750
find "$SATURN_STATE_DIR" \( -path "$SATURN_RELEASE_STAGING_DIR" -o -path "$SATURN_REPOSITORY_RESTORE_DIR" -o -path "$SATURN_DEPLOYMENT_STATE_DIR" \) -prune -o -type f -print0 \
  | xargs -0 -r chmod 0640
chmod 0750 "$SATURN_RELEASE_STAGING_DIR"
chmod 0750 "$SATURN_REPOSITORY_RESTORE_DIR"
ok "Permissions set"
...
MANIFEST_TOOL="$PRIVILEGED_SCRIPTS_DIR/$SATURN_RELEASE_MANIFEST_TOOL_NAME"
COMPONENTS_FILE="$SATURN_RELEASE_POLICY_DIR/$SATURN_RELEASE_COMPONENTS_NAME"
STATE_TOOL="$PRIVILEGED_SCRIPTS_DIR/$SATURN_STATE_COMPATIBILITY_TOOL_NAME"
STATE_ROOT="$SATURN_STATE_DIR"
STATE_BACKUP_ROOT="$SATURN_DEPLOYMENT_STATE_DIR/state-backups"
...
${SERVICE_USER} ALL=(root) NOPASSWD: ${PRIVILEGED_SCRIPTS_DIR}/${SATURN_RADIO_BACKEND_SWITCH_NAME} start p2
${SERVICE_USER} ALL=(root) NOPASSWD: ${PRIVILEGED_SCRIPTS_DIR}/${SATURN_RADIO_BACKEND_SWITCH_NAME} start xdma
${SERVICE_USER} ALL=(root) NOPASSWD: ${PRIVILEGED_SCRIPTS_DIR}/${SATURN_RADIO_BACKEND_SWITCH_NAME} start bridge
${SERVICE_USER} ALL=(root) NOPASSWD: ${PRIVILEGED_SCRIPTS_DIR}/${SATURN_RADIO_BACKEND_SWITCH_NAME} stop
${SERVICE_USER} ALL=(root) NOPASSWD: ${PRIVILEGED_SCRIPTS_DIR}/${SATURN_RADIO_BACKEND_SWITCH_NAME} stop p2
${SERVICE_USER} ALL=(root) NOPASSWD: ${PRIVILEGED_SCRIPTS_DIR}/${SATURN_RADIO_BACKEND_SWITCH_NAME} stop xdma
${SERVICE_USER} ALL=(root) NOPASSWD: ${PRIVILEGED_SCRIPTS_DIR}/${SATURN_RADIO_BACKEND_SWITCH_NAME} stop bridge
${SERVICE_USER} ALL=(root) NOPASSWD: ${PRIVILEGED_SCRIPTS_DIR}/${SATURN_APPLIANCE_POWER_NAME} schedule-poweroff
...
${SERVICE_USER} ALL=(root) NOPASSWD: ${PRIVILEGED_SCRIPTS_DIR}/saturn-tci-bind.sh *
${SERVICE_USER} ALL=(root) NOPASSWD: ${PRIVILEGED_SCRIPTS_DIR}/saturn-satp-config.sh
${SERVICE_USER} ALL=(root) NOPASSWD: ${PRIVILEGED_SCRIPTS_DIR}/saturn-satp-config.sh *
...
Environment=SATURN_MAINTENANCE_LOCK_DIR=${SATURN_MAINTENANCE_LOCK_DIR}
