//! Synchronization helpers for long-running bridge state.
//!
//! A panicking worker thread can poison a mutex. The bridge should report that
//! event, but RF control and cleanup paths still need access to the guarded
//! state so the operator can recover without a process crash.
    ...
    fn lock_unpoisoned(&self) -> MutexGuard<'_, T> {
        self.lock().unwrap_or_else(|poisoned| {
            if !MUTEX_POISON_WARNED.swap(true, Ordering::Relaxed) {
                eprintln!("saturn-bridge: WARN mutex poisoned; recovering guarded state");
            }
            poisoned.into_inner()
        })
    }
