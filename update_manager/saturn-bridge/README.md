...
- Mic audio received from TCI clients as binary TX-audio frames (standard
  `stream_type 2`; `stream_type 1` is RX audio and is rejected on ingress)
...
- **Commands received from client:**
  - Standard read forms (`vfo:0,0`, `dds:0`, `modulation:0`, and related
    commands) return current radio state without granting viewer clients control
  ...
  - Binary audio frame — mic audio for TX (`stream_type 2` only)
- **Messages sent to client:**
  - Standard TCI 2.0 initialization/capability sequence, full radio state, then
    `ready;` on connect
  - `tx_frequency:{hz}` — authoritative simplex/split TX frequency for
    amplifiers and other read-only accessories
  - Standard aliases for split, drive, AGC, NB/NR/ANF, TX permission, and sensors
...
The bridge installer also generates a machine-local FFTW wisdom cache through
Saturn's Rust FFTW planner and enables `saturn-fftw-wisdom.timer`. The weekly,
low-priority job validates a fingerprint of the bridge binary, FFTW library,
CPU identity, planner schema, and maximum transform size. It regenerates only
when that fingerprint changes or the cache is missing/corrupt. Saturn Bridge
imports `/var/cache/saturn-bridge/wdspWisdom01` before opening WDSP channels and
falls back to normal runtime planning if import fails.
...
| `SATURN_BRIDGE_TCI_HOST` | `127.0.0.1` | TCI WebSocket bind host; a specific non-loopback address also opens same-port loopback for Saturn Go |
| `SATURN_BRIDGE_FFTW_WISDOM_PATH` | unset in development | FFTW wisdom file imported before WDSP startup |
| `SATURN_BRIDGE_FFTW_WISDOM_MAX_SIZE` | `262144` | Largest power-of-two transform generated by the maintenance CLI |
